...
/**
 * RedDragon Deployment Recovery Script
 * Continues deployment from the LP Booster step
 */
async function main() {
  console.log("🚀 Starting RedDragon deployment recovery...");
  ...
  try {
    ...
    // Step 5: Deploy RedDragonLPBooster (retry)
    ...
    // Configure booster tiers - FIX: Make sure first tier matches global minimum
    console.log("Configuring LP Booster tiers...");
    
    // Set initial boost parameters using the same BigNumber instance
    console.log("Setting boost parameters...");
    await lpBooster.setBoostParameters(69, minLpAmount);
    
    // Add boost tiers (amounts in LP tokens) - FIXED: First tier matches global minimum exactly
    console.log("Adding tier 1 with exact same minLpAmount instance...");
    await lpBooster.addBoostTier(minLpAmount, 69);
    
    console.log("Adding remaining tiers...");
