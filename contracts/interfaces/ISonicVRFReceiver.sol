...
interface ISonicVRFReceiver {
    ...
    /**
     * @notice Event emitted when the DragonChainlinkVRF contract is updated
     * @param oldAddress The old contract address
     * @param newAddress The new contract address
     */
    event DragonChainlinkVRFUpdated(address oldAddress, address newAddress);
    
    /**
     * @notice Event emitted when an endpoint is set
     * @param chainId The chain ID
     * @param endpoint The endpoint address
     */
    event EndpointSet(uint16 chainId, address endpoint);
