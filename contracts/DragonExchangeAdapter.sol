...
contract DragonExchangeAdapter is IDragonExchangePair, Ownable, ReentrancyGuard {
     ...
     * Takes 9.31% of input wS tokens (6.9% to jackpot, 2.41% to ve69LP)
     * Burns 0.69% of resulting DRAGON tokens
     * @param user Address of the user doing the swap
    ...
    function swapWSForDragon(address user, uint256 wsAmount) external override nonReentrant returns (uint256 dragonAmount) {
        ...
        emit WSSwappedForDragon(user, wsAmount, userAmount);
        return userAmount;
    }
    
    /**
     * @dev Swap $DRAGON tokens for $wS tokens
     * Burns 0.69% of input DRAGON tokens
     * Takes 9.31% of resulting wS tokens (6.9% to jackpot, 2.41% to ve69LP)
     * @param user Address of the user doing the swap
     ...
     * @return liquidity Amount of BPT tokens received
     */
    function addLiquidity(
        ...
        bptToken.safeTransfer(user, liquidity);
     ...
     * @param liquidity Amount of BPT tokens to burn
    ...
    function removeLiquidity(
        ...
        bptToken.safeTransferFrom(user, address(this), liquidity);
        
        // Approve vault to use BPT
        bptToken.safeApprove(address(vault), liquidity);
        ...
        IBalancerVault.ExitPoolRequest memory request = IBalancerVault.ExitPoolRequest({
            ...
            userData: abi.encode(1, liquidity), // 1 = EXIT_KIND_EXACT_BPT_IN_FOR_TOKENS_OUT
            toInternalBalance: false
        });
        ...
        emit LiquidityRemoved(user, liquidity, wsAmount, dragonAmount);
