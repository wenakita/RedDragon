...
# Store API key and private key from config directory
...
gcloud secrets versions add sonicscan-api-key --data-file=./config/api_key.txt
...
gcloud secrets versions add dragon-private-key --data-file=./config/private_key.txt
...
if [ -d "metadata/sample-nft-images" ]; then
  echo "Uploading sample NFT images to Cloud Storage..."
  gsutil -m cp -r metadata/sample-nft-images/* gs://dragon-nft-assets-$project_id/whitelist/
fi
