...
IMAGE_NAME="us-central1-docker.pkg.dev/${PROJECT_ID}/${REPOSITORY}/${SERVICE_NAME}:temp-fix"

echo "🔵 Starting deployment of temporary fix for ${SERVICE_NAME}..."

# Build the image
echo "🔵 Building image using Cloud Build..."
gcloud builds submit ./integrations/cloud-run/temp-fix \
  --project=${PROJECT_ID} \
  --tag=${IMAGE_NAME} \
  --timeout=15m
